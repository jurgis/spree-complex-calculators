class Calculator::WeightAndQuantity < Calculator::Base
  ...
  def compute(order_or_line_items)
    ...
    weight_rate = get_rate(get_total_weight(line_items), ComplexCalculatorRate::WEIGHT)
    ...
    qnty_rate = get_rate(total_qnty, ComplexCalculatorRate::QNTY)
    # find the previous qnty rate or set it to 0 if not found
    qnty_rate = get_previous_rate(total_qnty, ComplexCalculatorRate::QNTY) || 0 if qnty_rate.nil?
    
    logger.info("@@@ The calculator #{name} weight_rate: #{weight_rate.inspect}, qnty_rate: #{qnty_rate.inspect}, total: #{weight_rate + qnty_rate}")
  ...
  def available?(order_or_line_items)
    ...
    logger.info("@@@ The calculator #{name} is available: #{available}")
    available
  end
